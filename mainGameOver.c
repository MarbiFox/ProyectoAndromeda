...
void navePintar(Nave*,SDL_Renderer*,Entidad**,Usuario*,int*,Entidad *,int *,int*);
...
void moverEntidades(SDL_Renderer *,SDL_Texture *,SDL_Texture *,SDL_Rect* ,Entidad**,int*,int*,int*,int*,SDL_Window*);
void entidadMover();
...
void dibujarMisilesBoss(Entidad *boss,Nave*naveUsuario,SDL_Renderer *renderer,Usuario *user,int *verificador,SDL_Rect *screenRectangle,SDL_Window*window){
    ...
    while (auxMisil != NULL){
        ...
        SDL_RenderDrawLine(renderer,auxMisil->x1,auxMisil->y1,auxMisil->x2,auxMisil->y2);
        colisionDisparo_Usuario(boss,naveUsuario,user,screenRectangle,renderer,window);
...
void startGame(){
    ...
    SDL_Surface *image3;
    image3 = IMG_Load("npc./boss00.jpg");
    if (!image3){
        printf("IMG_LOAD: %s",IMG_GetError());
    }

    SDL_Surface *image4;
    image4 = IMG_Load("npc./boss01.jpg");
    if (!image4){
        printf("IMG_LOAD: %s",IMG_GetError());
    }
    ...
    while (running){
        while (SDL_PollEvent(&event)){
            ...
            if (numeroentidades != 0){
                moverEntidades(renderer,imagen1,imagen2,&screenRectangle,matriz,&cont,&direccion_x,&direccion_y,&inicio,window);
            }
            SDL_SetRenderDrawColor(renderer,0,255,12,0);

            navePintar(&nave,renderer,matriz,user,&contadorBoss,boss,&numeroentidades,&confirmacion);
            if (contador == 0){
            ...
            if (confirmacion == 1){
                printf("CONFIRMACIONNNNNNNNNNNNNNNNNNNN\n");
                break;
            }
    }
...
void colisionDisparo_Usuario(Entidad *nave,Nave *naveUsuario,Usuario *user,SDL_Rect *screenRectangle,SDL_Renderer *renderer,SDL_Window*window){
    ...
    if (coordenadasEjeX-20 <= auxMisil->x1 && coordenadasEjeX+20 >= auxMisil->x1){
        if (coordenadasEjeY <= auxMisil->y1 && coordenadasEjeY+20 >= auxMisil->y1){
            ...
            if (user->vidas == 0){
                ...
                printf("[GAMEOVER]\n");
                gameover(screenRectangle,renderer,window);
            }
        }
    }
}

void dibujarMisilesEntidades(Entidad *nave,SDL_Renderer *renderer,int *cont, Nave *naveUsuario,Usuario *user,SDL_Rect*screenRectangle,SDL_Window*window){
    Misil *auxMisil = nave->misiles;
    while (auxMisil != NULL){
        ...
        SDL_RenderDrawLine(renderer,auxMisil->x1,auxMisil->y1,auxMisil->x2,auxMisil->y2);
        SDL_RenderDrawLine(renderer,auxMisil->x1,auxMisil->y1,auxMisil->x2,auxMisil->y2);
        colisionDisparo_Usuario(nave,naveUsuario,user,screenRectangle,renderer,window);
...
void colisionDisparo_Entidad(Misil *auxMisil,Entidad **matriz,Usuario *user,Nave *nave,int *numeroentidades,int *confirmacion){
...
void navePintar(Nave *nave,SDL_Renderer *renderer,Entidad**matriz,Usuario *user,int *contadorBoss,Entidad *boss,int *numeroentidades,int*confirmacion){
    SDL_Point points[8] = {
        {nave->x1,nave->y1},
        {nave->x2,nave->y2},
        {nave->x3,nave->y3},
    ...
    while (auxMisil != NULL){
        ...
        //Colisiones
        colisionDisparo_Entidad(auxMisil,matriz,user,nave,numeroentidades,confirmacion);
...
void mover_ejeY(Entidad **matriz,int *direccion_x,SDL_Rect*screenRectangle,SDL_Renderer*renderer,SDL_Window*window){
    for (int i = 0 ; i < 5 ; i++){
        for (int j = 0 ; j < 11 ; j++){
            if ((*direccion_x) == 1){
                matriz[i][j].rectangulo.x -= 8;
...
void moverEntidades(SDL_Renderer *renderer,SDL_Texture *imagen1,SDL_Texture *imagen2,SDL_Rect *screenRectangle,Entidad **matriz,int*cont,int *direccion_x,int *direccion_y,int *inicio,SDL_Window*window){
    ...
    for (int i = 0 ; i < 5 ; i++){
        for (int j = 0 ; j < 11 ; j++){
            if ((int)time % 2 == 0){
                ...
                //CASO INICIO
                if ((*cont) == 12 && (*inicio) == 1){
                    mover_ejeY(matriz,direccion_x,screenRectangle,renderer,window);
                    (*direccion_x) *= -1;
                    (*cont) = 0;
                    (*inicio) = 0;
                }
